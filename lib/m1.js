...
const drawImages = (doc, arrImage, typeGrid) => {
  ...
  //dims = [x, y, w, h];
  const dims = {
    "11": [roundDims([Xi, Yi, w, h])],
    "22h": [
      roundDims([Xi, Yi, w, h / 2 - pad]),
      roundDims([Xi, Yi + h / 2, w, h / 2 - pad])
    ],
    "22v": [
      roundDims([Xi, Yi, w / 2 - pad, h]),
      roundDims([Xi + w / 2, Yi, w / 2 - pad, h])
    ],
    "33c": [
      roundDims([Xi, Yi, w / 3 - 2 * pad, h]),
      roundDims([Xi + w / 3, Yi, (2 * w) / 3 - pad, h / 2 - pad]),
      roundDims([Xi + w / 3, Yi + h / 2, (2 * w) / 3 - pad, h / 2 - pad])
    ],
    "33h": [
    ...
    "33v": [
      roundDims([Xi, Yi, w / 3 - 2 * pad, h]),
      roundDims([Xi + w / 3, Yi, w / 3 - 2 * pad, h]),
      roundDims([Xi + (2 * w) / 3, Yi, w / 3 - 2 * pad, h])
    ],
    "44h": [
      roundDims([Xi, Yi, w / 2 - pad, h / 2 - pad]),
      roundDims([Xi + w / 2 + 2 * pad, Yi, w / 2 - pad, h / 2 - pad]),
    ...
    "44v": [
      roundDims([Xi, Yi, w / 4 - 3 * pad, h]),
      roundDims([Xi + w / 4 + pad, Yi, w / 4 - 3 * pad, h]),
      roundDims([Xi + (2 * w) / 4 + 2 * pad, Yi, w / 4 - 3 * pad, h]),
      roundDims([Xi + (3 * w) / 4 + 3 * pad, Yi, w / 4 - 3 * pad, h])
    ],
    "66": [
...
export const setImages = (doc, arrImage, configPos) => {
  ...
  switch (configPos) {
    case "11":
      partitionsImag = partitionArr(arrImage, 1);
      console.log(partitionsImag);
      drawImages(doc, partitionsImag, configPos);
      break;
    case "22h":
      partitionsImag = partitionArr(arrImage, 2);
      console.log(partitionsImag);
      drawImages(doc, partitionsImag, configPos);
      break;
    case "22v":
      ...
      break;
    case "33v":
      partitionsImag = partitionArr(arrImage, 3);
      console.log(partitionsImag);
      drawImages(doc, partitionsImag, configPos);
      break;
    case "33h":
